...
"""
    ...
    2. Various components of this folder are
        ...
        2.b metadata -> The metadata folder contains additional details are information that is needed for the bdinfo parser to function
            2.b.1 :: The metadata contains various .json files containing the metadata.
            2.b.2 :: The metadata folder also contains another folder named dump. The contents of the dump folder are the 
                    bdinfo expected to be generated by the parser. (pickle data) 
                    TODO: Figure out a way to store the data as a json or any human readable format.
        2.c working_folder -> The parser does some copy and move operations of certain files. 
    ...
    Test Setup
        ...
        3. Read the expected bdinfo file from the `bdinfo_metadata_dump` location.
...
bdinfo_metadata_dump = "/tests/resources/bdinfo/metadata/dump/"
...
def __get_expected_bd_info(file_name):
    try:
        return pickle.load(open(f'{working_folder}{bdinfo_metadata_dump}{file_name}.dump', "rb"))
    except:
        return None
