...
def choose_right_tracker_keys(config, tracker_settings, tracker):
        ...
        for optional_key, optional_value in optional_items.items():
            if str(optional_key) == str(translation_value):
                ...
                # for full disks the bdInfo is saved under the same key as mediainfo
                    ...
                    # else:
                    logging.debug(
                        f"Setting mediainfo from torrent_info to tracker_settings for optional_key {optional_key}")
                    ...
                    # else:
                    logging.debug(
                        "Skipping bdinfo for tracker settings since upload is NOT FullDisk.")
                else:
                    tracker_settings[optional_key] = torrent_info.get(
                        translation_key, "")
        # ------------ optional_items end ------------
    ...
    for default_key, default_value in config["Default"].items():
        logging.debug(
            f'Adding default key `{default_key}` with value `{default_value}` to tracker settings')
...
def identify_type_and_basic_info(full_path, guess_it_result):
    ...
    keys_we_need_torrent_info = ['screen_size',
                                 'source', 'audio_channels', 'type']
    ...
    for torrent in torrents:
        ...
        for tracker in upload_to_trackers:
            ...
            upload_status, upload_response = upload_to_site(
                upload_to=tracker, tracker_api_key=temp_tracker_api_key, config=config, tracker_settings=tracker_settings)
