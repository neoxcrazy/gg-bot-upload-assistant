...
def metadata_search_tmdb_for_id(query_title, year, content_type, auto_mode):
    ...
    if search_tmdb_request.ok:
        ...
        if len(search_tmdb_request.json()["results"]) == 0:
            logging.critical("[MetadataUtils] No results found on TMDB using the title '{}' and the year '{}'".format(
        ...
        logging.info(
            "[MetadataUtils] TMDB search has returned proper responses. Parseing and identifying the proper TMDB Id")
        logging.info(
        ...
        for possible_match in search_tmdb_request.json()["results"]:
            ...
            if content_type == "tv":
                ...
                if selected_year_sub_part == year or selected_year_sub_part == year - 1 or selected_year_sub_part == year + 1:
                    logging.debug(
                        f"[MetadataUtils] The possible match has passed the year filter")
                else:
                    logging.info(
                        f"[MetadataUtils] The possible match failed to pass year filter.")
        ...
        if result_num < 1:
            ...
            logging.info(
                f"[MetadataUtils] Cannot auto select a TMDB id. Marking this upload as TMDB_IDENTIFICATION_FAILED")
            return {
                "tmdb": "0",
                "imdb": "0",
                "tvmaze": "0",
        ...
        if auto_mode == 'false' and selected_tmdb_results > 1 and (os.getenv("auto_select_tmdb_result") or False):
            ...
            user_input_tmdb_id_num = Prompt.ask(
                "Input the correct Result #", choices=list_of_num, default="1")
            ...
            logging.info(
                f"[MetadataUtils] auto_mode is enabled so we are auto selecting #1 from tmdb results (TMDB ID: {str(result_dict[user_input_tmdb_id_num])})")
        else:
            console.print(
                "Cannot auto select a TMDB id. Marking this upload as [bold red]TMDB_IDENTIFICATION_FAILED[/bold red]")
            logging.info(
        ...
        imdb = str(metadata_get_external_id(id_site='tmdb', id_value=tmdb,
                   external_site="imdb", content_type=content_type))
        if content_type in ["episode", "tv"]:  # getting TVmaze ID
            ...
            tvmaze = str(metadata_get_external_id(
                id_site='imdb', id_value=imdb, external_site="tvmaze", content_type=content_type))
        else:
            tvmaze = "0"
        return {
            "tmdb": tmdb,
    ...
    try:
        if external_site == "imdb":  # we need imdb id
            if id_site == "tmdb":  # we have tmdb id
                logging.info(
                    f"[MetadataUtils] GET Request For IMDB Lookup: https://api.themoviedb.org/3/{content_type}/{id_value}/external_ids?api_key=<REDACTED>&language=en-US")
                ...
                if imdb_id_request["imdb_id"] is None:
                    ...
                    return "0"
                logging.debug(
                    f"[MetadataUtils] Returning imdb id as `{imdb_id_request['imdb_id']}`")
                return imdb_id_request["imdb_id"] if imdb_id_request["imdb_id"] is not None else "0"
            else:  # we have tvmaze
                logging.info(
                    f"[MetadataUtils] GET Request For IMDB Lookup: {get_imdb_id_from_tvmaze_url}")
                imdb_id_request = requests.get(
                    get_imdb_id_from_tvmaze_url).json()
                logging.debug(
                    f"[MetadataUtils] Returning imdb id as `{imdb_id_request['externals']['imdb']}`")
            ...
            if id_site == "imdb":
                logging.info(
                    f"[MetadataUtils] GET Request For TVMAZE Lookup: {get_tvmaze_id_from_imdb_url}")
                ...
                logging.error(
                    "[MetadataUtils] Cannot fetch tvmaze id without imdb id.")
                logging.debug("[MetadataUtils] Returning tvmaze id as `0`")
                return "0"
        else:  # we need tmdb id
            logging.info(
                f"[MetadataUtils] GET Request For TMDB Lookup: https://api.themoviedb.org/3/find/{id_value}?api_key=<REDACTED>&language=en-US&external_source=imdb_id")
            tmdb_id_request = requests.get(get_tmdb_id_from_imdb_url).json()
            for item in tmdb_id_request:
                if len(tmdb_id_request[item]) == 1:
                    logging.debug(
                        f"[MetadataUtils] Returning tmdb id as `{str(tmdb_id_request[item][0]['id'])}`")
                    return str(tmdb_id_request[item][0]["id"]) if tmdb_id_request[item][0]["id"] is not None else "0"
    except Exception as ex:
        logging.exception(
            "[MetadataUtils] Error while fetching external id. Returning `0` as the id")
        return "0"
...
def metadata_compare_tmdb_data_local(torrent_info):
    ...
    try:
        ...
        logging.exception(
            f'[MetadataUtils] Failed to get TVDB and MAL id from TMDB.')
    ...
    if "release_date" in get_media_info and len(get_media_info["release_date"]) > 0:
        ...
        logging.info(
            f"[MetadataUtils] Using the year we got from TMDB: {year}")
    return title, year, tvdb, mal
